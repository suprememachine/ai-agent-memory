...
const required = [
  "TELEGRAM_BOT_TOKEN", requiredLLMKey,
  "GITHUB_TOKEN", "GITHUB_OWNER", "GITHUB_REPO", "TELEGRAM_OWNER_ID",
];
...
async function main() {
  ...
  // ── OpenSea Monitor — buat SEBELUM bot agar bisa diteruskan ke createBot
  const opensea = new OpenSeaMonitor(sendMsg);

  // ── Telegram Bot — terima opensea monitor untuk wallet commands
  const bot = await createBot(agent, opensea);
  bot.start();
  ...
  app.get("/", (_, res) => res.json({
    status: "ok", provider,
    opensea_wallets: opensea.listWallets().length,
